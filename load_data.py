...
def load_char_vocab():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'char_vocab.txt')
    vocab = [line.strip() for line in open('char_vocab.txt', encoding='utf-8').readlines()]
...
def load_word_vocab():
    path = os.path.join(os.path.dirname(__file__), 'word_vocab.txt')
...
def load_word_embed(nb_words, embed_size):
    ...
    vocab = [line.strip() for line in open('word_vocab.txt', encoding='utf-8').readlines()]
    word2idx = {word: index for index, word in enumerate(vocab,start=1)}
    word2idx['PAD'] = 0
...
def load_char_data(file, data_size=None,maxlen=35):
    ...
    p = df['org'].values[0:data_size]
    h = df['org_change'].values[0:data_size]
    label = df['similarity'].values[0:data_size]
